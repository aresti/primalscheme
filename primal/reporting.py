...
class SchemeReporter(MultiplexScheme):
    ...
    def write_tsv(self, path='./'):
        ...
        with open(filepath, 'w') as tsvhandle:
            print(*['name', 'seq', 'length', '%gc', 'tm (use 65)'], sep='\t', file=tsvhandle)
            for r in self.regions:
                left = r.top_pair.left
                right = r.top_pair.right
                print(*map(str, [left.name, left.seq, left.length, '%.2f' %left.gc, '%.2f' %left.tm]), sep='\t', file=tsvhandle)
                print(*map(str, [right.name, right.seq, right.length, '%.2f' %right.gc, '%.2f' %right.tm]), sep='\t', file=tsvhandle)
                if r.alternates:
                    for alt in r.alternates:
                        print(*map(str, [alt.name, alt.seq, alt.length, '%.2f' %alt.gc, '%.2f' %alt.tm]), sep='\t', file=tsvhandle)
