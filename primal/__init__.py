...
class PoolOverlapException(Exception):
    """Ran into previous region in this pool"""
    pass


class SchemeOverlapException(Exception):
    """Ran into previous region in other pool"""
    pass
...
def find_primers(prefix, logger, amplicon_length, min_overlap, search_space, max_candidates, references, region_num, start_limits, debug):
    ...
    chunk_end = 2 * \
        amplicon_length if region_num == 1 else start_limits[1] + \
        amplicon_length
    ...
    start = 0 if region_num == 1 else start_limits[1] - \
        start_limits[0] - search_space
    p3_seq_args = {
    ...
    p3_global_args['PRIMER_PRODUCT_SIZE_RANGE'] = [
        [amplicon_length - allowed_variation, amplicon_length + allowed_variation]]
    ...
    while True:
        ...
        primer3_output = primer3.bindings.designPrimers(
            p3_seq_args, p3_global_args)
        ...
        if region_num == 1:
            ...
            logger.info("Stepping right, position %i, limit %s" %
                        (p3_seq_args[region_key][1], 'none'))
        # Step left (increase overlap)
        else:
            p3_seq_args[region_key][0] -= settings.STEP_DISTANCE
            p3_seq_args[region_key][1] += settings.STEP_DISTANCE
            logger.info("Stepping left, position %i, limit %i" % (
                p3_seq_args[region_key][0] + start_limits[0], start_limits[0]))

        # Check if we've run into the left limit
        if p3_seq_args[region_key][0] < 0:
            raise NoSuitableException(
                "No suitable primers found for region {} with current parameters. Try increasing amplicon length or reducing overlap.".format(region_num))
...
def multiplex(args, logger, parser=None):
    ...
    if args.amplicon_length < 100 or args.amplicon_length > 2000:
        logger.error(
            "--amplicon-length set to %s, must be within the range 100 to 2000" % (args.amplicon_length))
        raise ValueError(
            "--amplicon-length set to %s, must be within the range 100 to 2000" % (args.amplicon_length))
    ...
    while True:
        ...
        left_start_limit = results[-2].candidate_pairs[0].right.start + \
            1 if region_num > 2 else 0
        ...
        try:
            region = find_primers(args.prefix, logger, args.amplicon_length, args.min_overlap, args.search_space,
                                  args.max_candidates, references, region_num, (left_start_limit, right_start_limit), args.debug)
        ...
        if args.debug:
            ...
            for i in range(num):
                logger.debug(
                    region.candidate_pairs[0].left.alignments[i].formatted_alignment)
                logger.debug('Score:%i' % (
                    region.candidate_pairs[0].left.alignments[i].score))
                logger.debug(
                    region.candidate_pairs[0].right.alignments[i].formatted_alignment)
                logger.debug('Score:%i' % (
                    region.candidate_pairs[0].right.alignments[i].score))

        results.append(region)

        # Reporting
        logger.info("Region %i, %i:%i" % (
            region_num, results[-1].candidate_pairs[0].left.start, results[-1].candidate_pairs[0].right.start))
        if region_num > 1:
            ...
            logger.info("Product length %i, trimmed overlap %i" % (
                results[-1].candidate_pairs[0].product_length, trimmed_overlap))
        else:
            logger.info("Product length %i" %
                        (results[-1].candidate_pairs[0].product_length))
        if args.debug:
    ...
    plot_schemeadelica(args.prefix, logger, results,
                       references[0], args.output_path)
